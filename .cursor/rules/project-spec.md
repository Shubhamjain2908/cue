...
|---|---|---|---|
| Fetcher | `src/fetcher/` | Pulls OHLCV from Massive.com (`@massive.com/client-js`); news/earnings/overview from Alpha Vantage; caches to disk + SQLite | `pnpm run fetch` |
...
### 5.1  Massive.com — Stocks Aggregates (OHLCV)

Base URL: `https://api.massive.com`. The fetcher uses `@massive.com/client-js` (`restClient`) with `{ pagination: true }` so responses that include `next_url` are followed automatically on large date windows.

| Call | Role | Used In |
|---|---|---|
| `getStocksAggregates` | Daily OHLCV bars (`multiplier: 1`, `timespan: "day"`, `from` / `to` as `YYYY-MM-DD`, `adjusted: true`, `sort: "asc"`, high `limit` e.g. 50_000) | `src/fetcher/index.ts` |

Example (aligned with Massive JS client docs):

```typescript
import { restClient } from "@massive.com/client-js";

const rest = restClient(apiKey, "https://api.massive.com", { pagination: true });

await rest.getStocksAggregates({
  stocksTicker: "AAPL",
  multiplier: 1,
  timespan: "day",
  from: "2025-11-03",
...
| `@massive.com/client-js` | ^10.x | Massive REST SDK | `restClient` + `getStocksAggregates` against `https://api.massive.com`; `pagination: true` merges paged aggregate results. |
...
|---|---|---|
| Massive.com rate limit (429) | Log warning; fixed 12s spacing between tickers after each OHLCV request (free tier) | `src/fetcher/index.ts` |
